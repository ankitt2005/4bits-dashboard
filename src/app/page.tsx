...
export default function Dashboard() {
  ...
  // --- 4. CHAT SEND FUNCTION (LOGIC UPDATED FOR RENDER) ---
  const sendMessage = async () => {
    ...
    try {
      // ✅ UPDATED: Points to your live /api/chat endpoint
      const response = await axios.post(`https://causal-api-nry8.onrender.com/api/chat`, {
        message: input, 
      });

      // ✅ UPDATED: Maps correctly to backend response 'response' key
      const botMsg = { 
        sender: "bot", 
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: "⚠️ Connection error. Make sure your Render backend is awake!" },
      ]);
    }
